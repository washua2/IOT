<template>
    <div class="w-[45.125rem] h-[117.1875rem] relative transition-[left] duration-300 ease-in-out "
        :class="isLeft ? 'left-[-51.5rem]' : 'left-0'">
        <img src="@/assets/images/lod4/2.png" alt="" class="w-full h-full">
        <!-- video -->
        <div class="absolute top-[7.8125rem] left-0">
            <div
                ...
                <div class="w-[43.375rem] h-[24.8125rem]">
                    <VideoSmall class="w-full h-full" :elmId="'video13423213'" :videoUrl="videoMax.src" />
                </div>
        ...
        <div class="absolute top-[69.1875rem] left-0">
            <div id="realtimeEmotionTrend" class="w-[45.125rem] h-[24.625rem]"></div>
        </div>
        <!-- echart2 -->
        <div class="absolute bottom-[-1.25rem] left-0">
            <div id="proportion2" class="w-[45.125rem] h-[24.625rem]"></div>
        </div>
...
const { mode, setMode, ENVIRONMENTAL_VIDEO,ALAHLISAUDIFCFANSECTION } = useStreams()
const { AlertStore } = inject("store");
const isLeft = ref(true)
const videoMax = ref(ALAHLISAUDIFCFANSECTION)
...
onMounted(() => {
    ...
    renderEmotionChart('realtimeEmotionTrend')
    renderChart('proportion2')
})
</script>