<template>
  <div>
    ...
    <div class="h-[56rem] pt-[1rem] box-border relative left-0 top-[19.5rem] overflow-y-auto overflow-x-hidden scroll">
      <div  class="w-[40.9375rem] h-[30.75rem] relative left-[2.1875rem] top-0  rounded-md 
        shadow-[0rem_1.25rem_0.75rem_0rem_rgba(0,0,0,0.15)] flex-col justify-start items-start inline-flex"
          :class="isAlert ? 'breathing-border-active' : ''">
        <div
          class="breathing-border w-[40.9375rem] h-[30.75rem] relative left-[0] top-0 pt-[1.5rem] pl-[1.125rem] rounded-md 
        shadow-[0rem_1.25rem_0.75rem_0rem_rgba(0,0,0,0.15)] border-[0.0938rem] border-[#d03801] flex-col justify-start items-start inline-flex"
          style="background: radial-gradient(260.14% 141.21% at 52.54% 86.85%, rgba(52, 1, 1, 0.57) 0%, rgba(52, 1, 1, 0.00) 100%);">
          <div class="w-[38.375rem] h-[21.4375rem]">
            <VideoSmall class="w-full h-full" :elmId="'video11'" :videoUrl="videoMax.src" />
          </div>
          <div
            class="p-[0.5625rem] left-[36.25rem] top-[19.6875rem] absolute bg-black/50 rounded-full flex items-center justify-center cursor-pointer"
          ...
          <div class="text-white/90 text-2xl font-['light'] leading-[1.95rem] tracking-wide">{{ videoMax.addressId }}
          </div>
        </div>
      </div>
      ...
      <div
        ...
        <div
         ...
         bg-[#00111a]/25 rounded-md border border-white/10 flex-col justify-start items-start inline-flex mb-[1.8125rem] relative"
          ...
          <div
            class="p-[0.5625rem] left-[15.5719rem] top-[8.1719rem] absolute bg-black/50 rounded-full justify-start items-start inline-flex cursor-pointer"
    ...
    <div class="w-full absolute bottom-0 z-[10] bg-black/0 pt-[2rem]">
...
// import {SECURITY_CCTV_UPDATE,SECURITY_CCTV_UPDATE_MAX} from "/public/constants/url.js"
import { useStreams } from '@/store/modules/usestreams'

import { ref, inject,computed,watch } from "vue"
const { mode, setMode, SECURITY_CCTV, SECURITY_CCTV_UPDATE, SECURITY_CCTV_UPDATE_MAX } = useStreams()
const { AlertStore } = inject("store");
const videoMax = ref(SECURITY_CCTV_UPDATE_MAX)
const videoList = ref(SECURITY_CCTV_UPDATE)
const isAlert = ref(false)
const isVASAlertCount=computed(()=>AlertStore.state.isVASAlertCount)
watch(isVASAlertCount,()=>{
    isAlert.value=true
    setTimeout(()=>{
        isAlert.value=false
    },3000)
