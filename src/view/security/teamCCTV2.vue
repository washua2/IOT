<template>
  <div>
    <!-- <div
      class="w-[16.75rem] h-[55rem] relative rounded-lg border-2 border-white/20  overflow-hidden scaleUp">
      ...
      <div class="h-[51rem] flex items-center justify-start flex-col gap-[1.25rem] overflow-y-auto overflow-x-hidden scroll">
        <div
          ...
          <div class="w-[11.875rem] h-[6.625rem] rounded-md">
            <VideoSmall class="w-full h-full" :elmId="'video11112312'+index" :videoUrl="item.src"/>
          </div>
          <div
          ...
          <div
            class="text-white/90 text-[1.75rem] font-['light'] leading-9 tracking-wide mt-[1.375rem] mb-2">
            {{ item.title }}</div>
    ...
    <!-- </div> -->
    <div class="w-[16.75rem] h-[56.3125rem] rounded-[0.5rem] border-[0.125rem] border-white/20 overflow-hidden scaleUp">
      <!-- <div class="px-[2.125rem] pt-[1.25rem] text-[2rem] text-white font-['semibold'] tracking-[0.06rem] text-center">
        Team Delta Members
      </div>
      <div>
        <svg xmlns="http://www.w3.org/2000/svg" width="16.6875rem" height="0.1875rem" viewBox="0 0 267 3" fill="none">
          <path d="M0 1.81519H163.645H168.468H267" stroke="url(#paint0_linear_682_78209)" stroke-width="2" />
          <defs>
            <linearGradient id="paint0_linear_682_78209" x1="0.448761" y1="2.31047" x2="270.772" y2="33.809"
              gradientUnits="userSpaceOnUse">
              <stop stop-color="white" stop-opacity="0.06" />
              <stop offset="0.46303" stop-color="white" stop-opacity="0.72" />
              <stop offset="1" stop-color="white" stop-opacity="0" />
      ...
      <div class="w-[16.75rem] flex items-center justify-center flex-wrap gap-[1.25rem]">

        <div class="w-[14rem] h-[14.5rem] p-[1rem] rounded-[0.375rem] border-[0.0625rem] border-white/20"
          style="background: rgba(0, 17, 26, 0.27);" v-for="(item, index) in cctvList" :key="index">
          <div class="w-[11.875rem] h-[9.75rem] rounded-[0.375rem] overflow-hidden bg-[#FBFBF4] opacity-[0.8]">
            <VideoSmall class="w-full h-full" :elmId="'video11112312' + index" :videoUrl="item.src" />
          </div>
          <div class="w-full mt-[0.625rem] text-center justify-center text-white text-[1.75rem] font-['light'] leading-9 tracking-wide 
          [text-shadow:_0px_4px_4px_rgb(0_0_0_/_0.54)]">{{ item.title }}</div>
        </div>
      </div> -->
      <img src="@/assets/images/personal/1.png" class="w-full h-full" alt="">
